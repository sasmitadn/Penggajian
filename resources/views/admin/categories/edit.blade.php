    ...
    <div class="container">
        <div class="page-inner">
            <div class="page-header">
                <h3 class="fw-bold mb-3">Edit Jabatan</h3>
            ...
            <form action="{{ route('admin.' . $menu . '.update', [$menu, $data->id]) }}" method="post"
                    ...
                    <div class="col-md-12">
                        <div class="card">
                                ...
                                <div class="row">
                                    <div class="col-md-6 col-lg-4">
                                        <x-form-default name="name" label="Nama" type="text"
                                            old="{{ $data->name }}" />
                                        <x-form-radio name="status" label="Status" selected="{{ $data->status }}"
                                            :options="['active' => 'Aktif', 'inactive' => 'Tidak Aktif']" />
                                        <x-form-radio name="is_paid" label="Kategori" selected="{{ $data->is_paid }}"
                                            :options="['1' => 'Dibayar', '0' => 'Tidak Dibayar']" />
                                    </div>
                                    <div class="col-md-6 col-lg-4 hiddenLayout">
                                        <x-form-default name="price_daily" label="Gaji Harian" type="number"
                                            old="{{ $data->price_daily }}" />
                                        <x-form-default name="price_overtime" label="Gaji Lembur" type="number"
                                            old="{{ $data->price_overtime }}" />
                                    </div>
                                    <div class="col-md-6 col-lg-4 hiddenLayout">
                                        <x-form-default name="work_start" label="Jam Kerja Dimulai" type="time"
                                            old="{{ $data->work_start }}" />
                                            ...
                                            @foreach (Config::get('role') as $feature => $actions)
                                                <tr>
                                                    ...
                                                    @foreach ($actions as $action => $routes)
                                                        <td>
                                                            @if (count($routes) != 0)
                                                                <input type="checkbox" value="{{ json_encode($routes) }}"
                                                                    name="role[]" class="accessCheckbox"
                                                                    @if (old('role') == null) @if (!empty(array_intersect($routes, json_decode($data->role ?? '[]')))) checked @endif
                                                                @else
                                                                    @foreach (old('role') as $item)
                                                                        {{ !empty(array_intersect($routes, json_decode($item))) ? 'checked' : '' }} @endforeach
                                                                    @endif>
                                                            @endif
                                                        </td>
                                                    @endforeach
                            ...
                            <div class="card-action">
                                ...
                                <a href="{{ route('admin.' . $menu . '.index', [$menu]) }}" class="btn btn-danger">Hapus</a>
                            </div>
                        </div>
                    </div>
                </div>
