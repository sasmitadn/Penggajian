...
<div class="offcanvas offcanvas-end" tabindex="-1" id="filterSidebar">
    ...
    <div class="offcanvas-body">
        <form action="{{ route('admin.cash_advances.index') }}" method="get" id="filterForm">
            @csrf
            <x-form-search class="mb-3" name="id_user" label="Pilih User" selected="{{ $request->id_user }}"
                :options="$users->pluck('name', 'id')" />
            <x-form-search class="mb-3" name="id_category" label="Pilih Jabatan" selected="{{ $request->id_category }}"
